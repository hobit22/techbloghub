...
public interface PostRepository extends JpaRepository<Post, Long> {
    ...
    @Query("SELECT p FROM Post p JOIN p.tags t WHERE t.name IN :tagNames ORDER BY p.publishedAt DESC")
    Page<Post> findByTagNames(@Param("tagNames") List<String> tagNames, Pageable pageable);
    
    @Query("SELECT p FROM Post p JOIN p.categories c WHERE c.name IN :categoryNames ORDER BY p.publishedAt DESC")
