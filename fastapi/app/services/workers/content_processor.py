...
class ContentProcessor:
    ...
    async def get_pending_posts(self, limit: int = 10) -> List[Post]:
        ...
        return await self.repository.get_pending_posts(
            limit=limit, max_retry_count=self.max_retries
        )
    ...
    async def process_post(self, post: Post, skip_commit: bool = False) -> dict:
        ...
        try:
            ...
            if not extracted:
                ...
                if extracted_title and len(extracted_title.strip()) >= len(
                    post.title.strip()
                ):
                    post.title = extracted_title.strip()
                    ...
                    if extracted_date:
                        try:
                            ...
                            if isinstance(extracted_date, str):
                                ...
                                logger.info(
                                    f"Post {post.id}: Using Trafilatura date - {post.published_at}"
                                )
                            ...
                            logger.warning(
                                f"Post {post.id}: Failed to parse Trafilatura date '{extracted_date}': {e}"
                            )
                    else:
                        logger.warning(
                            f"Post {post.id}: No published_at from RSS and Trafilatura"
                        )
                ...
                result["method"] = extracted.get("method")
    ...
    async def process_pending_batch(
        self, batch_size: int = 10, max_concurrent: int = 5
    ) -> dict:
        """
        ...
        """
        summary = {"total_processed": 0, "completed": 0, "failed": 0, "errors": []}
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                summary["errors"].append(
                    {"post_id": None, "url": None, "error": str(result)}
                )
        ...
        try:
            await self.db.commit()
            logger.info(
                f"Batch commit successful: {summary['total_processed']} posts processed"
            )
        except Exception as e:
    ...
    async def retry_failed_posts(
        self, batch_size: int = 5, max_concurrent: int = 3
    ) -> dict:
        """
        실패한 포스트들 병렬 재시도
        ...
        Returns:
            처리 결과 요약
        """
        summary = {"total_retried": 0, "completed": 0, "failed": 0, "errors": []}
        ...
        # 병렬 재시도
        results = await asyncio.gather(
            *[retry_with_limit(post) for post in failed_posts], return_exceptions=True
        )
        ...
        for result_tuple in results:
            if isinstance(result_tuple, Exception):
                ...
                result, retry_count = cast(tuple[dict[str, Any], int], result_tuple)
                summary["total_retried"] += 1
        ...
        try:
            await self.db.commit()
            logger.info(
                f"Retry batch commit successful: {summary['total_retried']} posts retried"
            )
        except Exception as e:
            await self.db.rollback()
